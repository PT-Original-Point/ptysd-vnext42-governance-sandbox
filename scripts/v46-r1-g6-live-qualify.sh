#!/bin/sh
set -eu
BASE="$HOME/.local/state/ptysd-v46-r1g6"
WORK="$BASE/workspace"
...
printf 'PENDING\n' > "$WORK/src/message.txt"
cat > "$WORK/tests/test_message.py" <<'PY'
...
if actual != expected:
    ...
    sys.exit(1)
print("PASS_R1G6_SYNTHETIC_MESSAGE")
PY
cat > "$WORK/TASK.md" <<'EOF'
# R1-G6 synthetic qualification task
Change exactly one file: `src/message.txt`.
Required final bytes: `HELLO_FROM_OPENCODE\n`
...
printf 'PASS_R1G6_REAL_MODEL_DYNAMIC_ZERO_COST\n'
...
printf 'AUTH_PROVIDER_PRESENT=true\n'
printf 'WORKSPACE_CLEANUP=ON_EXIT\n'
