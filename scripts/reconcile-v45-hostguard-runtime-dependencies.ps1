...
function Get-EnabledWinRmPortRules {
  ...
  foreach($r in $inbound){foreach($pf in @($r|Get-NetFirewallPortFilter -ErrorAction SilentlyContinue)){if(([string]$pf.LocalPort) -in @('5985','5986')){$hits+=$r}}}
  return @($hits)
}
function Get-LoopbackListenerState {
  ...
  $addresses=@();foreach($v in @($listener.ListeningOn)){foreach($a in @(([string]$v)-split '\s*,\s*')){if(-not [string]::IsNullOrWhiteSpace($a)){$addresses+=$a.Trim()}}}
...
function Get-RequiredModuleNames($data) {
  $names=@();foreach($entry in @($data.RequiredModules)){if($entry -is [string]){$names+=[string]$entry}elseif($null -ne $entry.ModuleName){$names+=[string]$entry.ModuleName}}
  @($names)
}
...
try{
  ...
  $checks=@{'Get-VM'='Hyper-V';'Get-CimInstance'='CimCmdlets';'Test-NetConnection'='NetTCPIP'}
  ...
  [pscustomobject]@{Result='HOSTGUARD_RUNTIME_DEPENDENCIES_RECONCILED';SourceCommit=$SourceCommit;Host=$env:COMPUTERNAME;VmName=$vm.Name;VmId=$vm.Id.ToString();RequiredModules=$ExpectedRequired;GetVmResolved=$true;GetCimInstanceResolved=$true;TestNetConnectionResolved=$true;ListenerAddress=$listenerPost.Address;ListenerTransport=$listenerPost.Transport;ListenerPort=$listenerPost.Port;ListeningOn=$listenerPost.ListeningOn;BackupManifest=$BackupManifest;VmMutation=$false;PublicFirewallMutation=$false;RunnerPrivilegeElevation=$false;GlobalExecutionPolicyMutation=$false}|ConvertTo-Json -Depth 4
}catch{
