...
$names=@();foreach($entry in $required){if($entry -is [string]){$names+=[string]$entry}elseif($null -ne $entry.ModuleName){$names+=[string]$entry.ModuleName}}
...
[pscustomobject]@{Result='PASS_V45_HOSTGUARD_RUNTIME_DEPENDENCIES';RequiredModules=$names;HyperVModuleLoaded=[bool](Get-Module Hyper-V);CimCmdletsModuleLoaded=[bool](Get-Module CimCmdlets);NetTCPIPModuleLoaded=[bool](Get-Module NetTCPIP);GetVmResolved=$true;GetCimInstanceResolved=$true;TestNetConnectionResolved=$true}|ConvertTo-Json -Depth 4
