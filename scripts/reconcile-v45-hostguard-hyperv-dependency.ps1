...
$BackupManifest=Join-Path $ModuleTarget 'PTYSD.HostGuard.psd1.pre-hyperv-dependency-reconcile'
...
if($currentRequired.Count -eq 1 -and $currentRequired[0] -eq 'Hyper-V'){
  [pscustomobject]@{Result='HOSTGUARD_HYPERV_DEPENDENCY_ALREADY_RECONCILED';SourceCommit=$SourceCommit;Host=$env:COMPUTERNAME;VmName=$vm.Name;VmId=$vm.Id.ToString();RequiredModule='Hyper-V';VmMutation=$false;PublicFirewallMutation=$false;RunnerPrivilegeElevation=$false}|ConvertTo-Json -Depth 3
  exit 0
}
if($currentRequired.Count -ne 0){throw ('HOSTGUARD_REQUIRED_MODULE_PRESTATE_UNEXPECTED actual='+($currentRequired -join ','))}
if(Test-Path -LiteralPath $BackupManifest){throw 'HYPERV_DEPENDENCY_RECONCILE_BACKUP_ALREADY_EXISTS_MANUAL_REVIEW'}

$stage=Join-Path $env:TEMP ('ptysd-hostguard-hyperv-'+[Guid]::NewGuid().ToString('N'))
...
if($candidateRequired.Count -ne 1 -or $candidateRequired[0] -ne 'Hyper-V'){throw ('CANDIDATE_REQUIRED_MODULE_INVALID actual='+($candidateRequired -join ','))}
...
try{
  ...
  if($installedRequired.Count -ne 1 -or $installedRequired[0] -ne 'Hyper-V'){throw ('INSTALLED_REQUIRED_MODULE_INVALID actual='+($installedRequired -join ','))}
  ...
  if($null -eq (Get-Module Hyper-V -ErrorAction SilentlyContinue)){throw 'HYPERV_REQUIRED_MODULE_NOT_LOADED_AFTER_INSTALL'}
  $getVm=Get-Command Get-VM -ErrorAction Stop
  if([string]$getVm.ModuleName -ne 'Hyper-V'){throw "GET_VM_SOURCE_INVALID actual=$($getVm.ModuleName)"}
  ...
  [pscustomobject]@{
    Result='HOSTGUARD_HYPERV_DEPENDENCY_RECONCILED'
    ...
    GetVmResolved=$true
    GetVmModule=[string]$getVm.ModuleName
  ...
  if($rollbackErrors.Count -gt 0){throw ("HYPERV_DEPENDENCY_RECONCILE_FAILED=$err;ROLLBACK_ERRORS="+($rollbackErrors -join '|'))}
  throw $err
}finally{
