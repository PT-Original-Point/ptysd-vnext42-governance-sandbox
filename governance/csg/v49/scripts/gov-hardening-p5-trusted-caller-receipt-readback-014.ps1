...
[ordered]@{schema='v49.p5.trusted-caller-receipt-reconciliation.v1';readback_only=$true;receipts=@(Receipt $ids[0],Receipt $ids[1]);canary_dir_present=[bool](Test-Path -LiteralPath $dir);canary_js_present=[bool](Test-Path -LiteralPath $js -PathType Leaf);stdout_file_present=[bool](Test-Path -LiteralPath ($js+'.out') -PathType Leaf);stderr_file_present=[bool](Test-Path -LiteralPath ($js+'.err') -PathType Leaf);matching_processes=$procs;no_output_content_read=$true;no_file_delete=$true;no_process_kill=$true;observed_at_utc=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json -Depth 7 -Compress
