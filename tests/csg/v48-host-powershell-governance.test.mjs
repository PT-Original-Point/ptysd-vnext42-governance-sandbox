...
test('governed SYSTEM host PowerShell is retained as fourth Factory MCP tool with V4.8 fence',()=>{
  ...
  assert.ok(upgrade.includes('foreach ($rel in $candidateFiles)'));
  assert.ok(upgrade.includes('elseif ($rel -in $newFiles)'));
  assert.ok(upgrade.includes("& 'C:\\Program Files\\nodejs\\node.exe' (Join-Path $install 'tests\\protocol-smoke.mjs')"));
  assert.doesNotMatch(upgrade,/npm\.cmd' test/);
