...
test('repair requires prior Hyper-V-only prestate and upgrades exact dependency set',()=>{
  assert.match(repair,/HOSTGUARD_REQUIRED_MODULE_PRESTATE_UNEXPECTED/);
  assert.match(repair,/ExpectedRequired=@\('Hyper-V','CimCmdlets','NetTCPIP'\)/);
  assert.match(repair,/CANDIDATE_REQUIRED_MODULES_INVALID/);
  assert.match(repair,/INSTALLED_REQUIRED_MODULES_INVALID/);
  assert.match(repair,/Get-Command \$name/);
});

test('repair preserves endpoint and host security boundaries',()=>{
  assert.match(repair,/PSSC_EXECUTION_POLICY_NOT_BYPASS/);
  assert.match(repair,/RestrictedRemoteServer/);
  assert.match(repair,/NoLanguage/);
